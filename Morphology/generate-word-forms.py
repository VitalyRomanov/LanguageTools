...
def convert_gramma(gramma):
    ...
    if pos in ['NN','NNN','NNF','NNP']: # <---------- change
        if pos =='NN':
            output = 'NN:Inanim'
        elif pos =='NNN':
            output = 'NN:Name'
...
# os.system("time java -jar languagetool-commandline.jar -l ru -eo -e Unknown_words  unique-tag.txt > out.txt")
# news = set()
...
os.system("./mystem -ni --eng-gr exactly_1.txt mout.txt")
...
with open('mout.txt','r') as new_file:
    for line in new_file:
        if not "?" in line:
            ...
            mystems.append("".join(filter(lambda x: x != '?', line)))
