...
vocab_words = open("../for review/wiki_proper/wiki_tokens", "r").read().split("\n")

vocab_words = list(map(lambda x: x.split(" ")[0], vocab_words))
...
def extract_patterns_in_words(patterns, word1, word2, max_len):
    ...
    if i != 1 and i > max(len(word1[i - 1:]), len(word2[i - 1:])) < max_len:
        if ("prefix", word1[:i-1]) in patterns:
            patterns[("prefix", word1[:i-1])][0] += 1
            # patterns[("prefix", word1[:i-1])][1].append((word1,word2))
        else:
            patterns[("prefix", word1[:i-1])] = [1,[(word1,word2)]]
            #         patterns[("suffix",word1[i-1:], word2[i-1:], word1, word2)] += 1
    i = 1
    while word1[-i:] == word2[-i:]:
        i = i + 1
    if i != 1 and max(len(word1[:-i + 1]), len(word2[:-i + 1])) < max_len:
        if ("suffix", word1[-i+1:]) in patterns:
            patterns[("suffix", word1[-i+1:])][0] += 1
            # patterns[("suffix", word1[-i+1:])][1].append((word1,word2))
        else:
            patterns[("suffix", word1[-i+1:])] = [1,[(word1,word2)]]
            ...
            with open(patterns_dir + filename, 'rb') as handle:
                ...
                for key in result.keys():
                    if key in patterns:
                        ...
                        # print(patterns[key])
                        # print(result[key])
                        patterns[key][0] += result[key][0]
                        patterns[key][1].extend(result[key][1])
                    ...
                    # print(repr(key), "\t", patterns[repr(key)])
        for p in patterns.most_common(100):
            print(p[0], p[1][0])
